...
namespace FireApp.Domain
{
    ...
    public class CopyOfUserGroup : IEquatable<CopyOfUserGroup>
    {
        protected CopyOfUserGroup() { }

        protected CopyOfUserGroup(int id, string name)
        {
            this.Id = id;
            this.Name = name;
        }
        ...
        /// <param name="csv">A line of a CSV-File you want to convert.</param>
        /// <returns>Returns a new ServiceGroup or null if an error occures.</returns>
        public static CopyOfUserGroup GetFromCsv(string csv)
        {
            string[] values;
            ...
            if (csv != null)
            {
                ...
                {
                    values = csv.Split(';');
                    CopyOfUserGroup ug = new CopyOfUserGroup(Convert.ToInt32(values[0]), values[1]);
                    return ug;
                }
                catch (Exception)
                {
        ...
        public bool Equals(CopyOfUserGroup other)
        {
            if (this.Id == other.Id)
            {
                return true;
